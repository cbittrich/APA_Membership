...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $input_Terminationdate = trim($_POST["Terminationdate"]);
    if(empty($input_Terminationdate)){
        $Terminationdate_err = "Please enter an Terminationdate.";     
    } else{
        $Terminationdate = $input_Terminationdate;
    }
    

    
    // Check input errors before inserting in database
    if(empty($Terminationdate_err)){
        // Prepare an update statement
        $sql = "UPDATE User SET User_Effdt=now(), User_Position_TerminationDate=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "si",  $param_Terminationdate, $param_id);
            
            // Set parameters
            $param_Terminationdate = $Terminationdate;
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    $Terminationdate = $row["User_Position_TerminationDate"];
                } else{
...
<body>
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="mt-5">Terminate Employee: <?php echo $firstname; ?> <?php echo $lastname; ?>?</h2>
                    <p>Please select the employee termination date value and submit to update the employee record.</p>
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group">
                            <label>Termination date:</label>
                            <input type="Date" name="Terminationdate" class="form-control <?php echo (!empty($Terminationdate_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $Terminationdate; ?>">
                            <span class="invalid-feedback"><?php echo $Terminationdate_err;?></span>
                        </div>
