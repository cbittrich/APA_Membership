...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    // Validate User_status_Effdt User_status_Effdt
    $input_User_status_Effdt = trim($_POST["User_status_Effdt"]);
    if(empty($input_User_status_Effdt)){
        $User_status_Effdt_err = "Please enter an User_status_Effdt.";     
    } else{
        $User_status_Effdt = $input_User_status_Effdt;
    }
    

    
    // Check input errors before inserting in database
    if(empty($User_status_Effdt_err)){
        // Prepare an update statement
        $sql = "UPDATE User_Status SET User_status_Current='Withdrawn', User_status_Effdt=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "si",  $param_User_status_Effdt, $param_id);
            
            // Set parameters
            $param_User_status_Effdt = $User_status_Effdt;
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM User_status_Effdt WHERE id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Please select the membership termination date value and submit to update the membership record.</p>
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group">
                            <label>Current Status:</label>
                            <input type="Date" name="User_status_Effdt" class="form-control <?php echo (!empty($User_status_Effdt_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $User_status_Effdt; ?>">
